...
/**
 * Folder-index icon (small arrow-up-right inside a rounded box).
 * Rendered next to each folder's name in the sidebar to act as a
 * "jump to this folder's index page" affordance. The anchor that
 * wraps it stops click propagation so the folder header's
 * expand/collapse handler doesn't fire.
 */
const iconFolderIndex = `
<svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
  <rect x="2" y="2" width="20" height="20" rx="4" fill="#1E88E5"/>
  <path d="M9 15 L15 9 M11 9 H15 V13" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
</svg>`;
 ...
 * (e.g. `/pages/multimedia/index.html`) -- strip the `/index.html`
 * suffix so the address bar shows the clean directory URL.
 *
 * This applies to the auto-generated folder indexes that the build
 * creates for every subfolder of `pages/`. After the redirect fires
 * the user lands on the directory URL (e.g. `/pages/multimedia/`),
 * which is what the sidebar's "jump to folder index" affordance and
 * the auto-generated nested folder list already point at conceptually.
 * The relative links inside the auto-generated folder list resolve the
 * same way from both the file URL and the directory URL, so nothing
 * else has to change.
 *
 * Runs synchronously at script load so the redirect happens before the
 * page paints.
 */
(function redirectIndexHtml() {
  const url = window.location.href;
  if (url.endsWith("/index.html")) {
    // Compute the parent-directory URL by chopping off the trailing
    // "/index.html" and keeping the slash so the result is the clean
    // directory URL ("/" for the root, "file:///C:/path/" for file://,
    // "/pages/multimedia/" for a subfolder, etc.).
    const parent = url.substring(0, url.length - "/index.html".length);
    // Preserve any hash (e.g. /index.html#section) so the user lands
    // on the same anchor on the parent page.
    const hash = window.location.hash || "";
    window.location.replace(parent + hash);
  }
})();
...
function highlightOnLoad() {
  // Only highlight when the URL contains a hash that matches a header id.
    ...
    .then(indexData => {
      ...
      function renderNode(node, container) {
        ...
        if (node.type === "folder") {
          ...
          // "Jump to folder index" affordance: a small icon at the right
          // of the header that links to the folder's own index.html
          // (auto-generated by the build for every subfolder of pages/).
          // We use an <a> wrapper so middle-click / cmd+click work the
          // way users expect, and stopPropagation so the header's
          // expand/collapse handler doesn't fire.
          const folderIndexLink = document.createElement("a");
          folderIndexLink.classList.add("folder-index-link");
          folderIndexLink.href = prefix + "pages/" + node.path + "/";
          folderIndexLink.title = "Go to " + node.name + " folder";
          folderIndexLink.setAttribute("aria-label", "Go to " + node.name + " folder");
          folderIndexLink.innerHTML = iconFolderIndex;
          folderIndexLink.addEventListener("click", (e) => {
            // Don't toggle the folder when the user clicks the link.
            e.stopPropagation();
          });
          header.appendChild(folderIndexLink);
