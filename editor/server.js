  ...
  try {
    if (fs.existsSync(CONFIG_PATH)) {
      ...
      for (const line of raw.split("\n")) {
        ...
        // Store raw value (with quotes if present)
        config._raw[key] = val;
        // Strip surrounding quotes for parsed values
...
async function handleRequest(req) {
  ...
  if (url.pathname === "/api/config/save" && method === "POST") {
    ...
    if (!newConfig) {
      return jsonResponse({ error: "Missing config data" }, 400);
    }
    ...
    try {
      ...
      // Get the raw values from the current config to know original formatting
      const currentConfig = getProjectConfig();

      // Update values while preserving comments and formatting
      ...
      const updatedLines = lines.map((line) => {
        ...
        if (key in newConfig) {
          ...
          let val = String(newConfig[key]);
          // Preserve quotes from the original raw value if they were present
          const rawVal = currentConfig._raw?.[key];
          if (rawVal && ((rawVal.startsWith('"') && rawVal.endsWith('"')) || (rawVal.startsWith("'") && rawVal.endsWith("'")))) {
            val = `"${val}"`;
          }
      ...
      // If config file was empty, create it from scratch
      let outputContent;
      if (updatedLines.length === 0 || (updatedLines.length === 1 && updatedLines[0] === "")) {
        outputContent = `# Project Configuration\n# Generated by MMX Visual Editor\n\n`;
        for (const [key, val] of Object.entries(newConfig)) {
          outputContent += `${key} = ${val}\n`;
        }
      } else {
        outputContent = updatedLines.join("\n");
      }

      fs.writeFileSync(CONFIG_PATH, outputContent, "utf-8");
      return jsonResponse({ success: true, path: CONFIG_PATH });
    } catch (e) {
      return jsonResponse({ error: e.message }, 500);
    }
  }

  // GET /api/doc-config — get documentation config (input/config.mcfg)
  if (url.pathname === "/api/doc-config" && method === "GET") {
    try {
      const config = getProjectConfig();
      const inputDir = config.inputPath || path.join(PROJECT_ROOT, "1Example/input");
      const docConfigPath = path.join(inputDir, "config.mcfg");
      const result = {
        title: "MMX",
        version: "",
        lang: "en",
        sidebarBottomText: "",
        defaultCodeHighlight: false,
        noDefaultIndex: false,
        _raw: {},
      };
      if (fs.existsSync(docConfigPath)) {
        const raw = fs.readFileSync(docConfigPath, "utf-8");
        for (const line of raw.split("\n")) {
          const trimmed = line.trim();
          if (trimmed.startsWith("#") || trimmed === "") continue;
          const eqIdx = trimmed.indexOf("=");
          ...
          else if (key === "lang") result.lang = val;
          else if (key === "sidebarBottomText") result.sidebarBottomText = val;
          else if (key === "defaultCodeHighlight") result.defaultCodeHighlight = val === "true";
          else if (key === "noDefaultIndex") result.noDefaultIndex = val === "true";
        }
      }
  ...
  // POST /api/doc-config/save — save documentation config (input/config.mcfg)
  if (url.pathname === "/api/doc-config/save" && method === "POST") {
    const { config: newConfig } = body || {};
    if (!newConfig) {
      return jsonResponse({ error: "Missing config data" }, 400);
    }
    ...
    try {
      ...
      const inputDir = projectConfig.inputPath || path.join(PROJECT_ROOT, "1Example/input");
      const docConfigPath = path.join(inputDir, "config.mcfg");

      // Read the existing config file
      let configContent = "";
      if (fs.existsSync(docConfigPath)) {
        configContent = fs.readFileSync(docConfigPath, "utf-8");
      }

      // Parse existing to get raw values for quote preservation
      const currentDocConfig = { _raw: {} };
      for (const line of configContent.split("\n")) {
        ...
        if (eqIdx === -1) continue;
        const key = trimmed.slice(0, eqIdx).trim();
        currentDocConfig._raw[key] = trimmed.slice(eqIdx + 1).trim();
      }
      ...
      const updatedLines = lines.map((line) => {
        ...
        if (key in newConfig) {
          const indent = line.slice(0, line.indexOf(trimmed));
          let val = String(newConfig[key]);
          const rawVal = currentDocConfig._raw?.[key];
          if (rawVal && ((rawVal.startsWith('"') && rawVal.endsWith('"')) || (rawVal.startsWith("'") && rawVal.endsWith("'")))) {
            val = `"${val}"`;
          }
          return `${indent}${key} = ${val}`;
      ...
      let outputContent;
      if (updatedLines.length === 0 || (updatedLines.length === 1 && updatedLines[0] === "")) {
        outputContent = `# Documentation Configuration\n# Generated by MMX Visual Editor\n\n`;
        for (const [key, val] of Object.entries(newConfig)) {
          outputContent += `${key} = ${val}\n`;
        }
      } else {
      ...
      fs.writeFileSync(docConfigPath, outputContent, "utf-8");
      return jsonResponse({ success: true, path: docConfigPath });
    } catch (e) {
      return jsonResponse({ error: e.message }, 500);
    }
  }
